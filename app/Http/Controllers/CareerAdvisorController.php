...
Tu es un expert en analyse de CV.tu fera en fontion de la langue que tu vas identifier , IMPORTANT: Tu dois analyser le texte fourni et retourner UNIQUEMENT un objet JSON valide suivant EXACTEMENT cette structure, sans texte avant ou après:
{
    ...
    "hobbies": [
        "string (centre d'intérêt 1)",
        "string (centre d'intérêt 2)"
    ],
    "formation": "string (formation académique principale ou diplôme le plus élevé)"
}
...
8. Les compétences doivent être extraites du CV et listées individuellement , 5 competences max
9. Les hobbies/centres d'intérêt doivent être extraits du CV et listés individuellement , 5 hobbies max
10. La formation doit être le diplôme le plus élevé ou la formation académique principale
EOT;
    }
    ...
    private function saveCVData($userId, $cvData)
    {
        ...
        try {
            ...
            // Traiter la formation (comme une profession manuelle)
            if (!empty($cvData['formation'])) {
                $user->update([
                    'full_profession' => $cvData['formation'] // On peut remplacer par la formation
                ]);
            }
            ...
            // Sauvegarder les compétences manuelles
            if (!empty($cvData['competences'])) {
                ...
                foreach ($cvData['competences'] as $competenceName) {
                    ...
                    $manualCompetences[] = [
                        'id' => 'manual-' . uniqid(),
                        'name' => $competenceName,
                        'name_en' => $competenceName, // Même valeur par défaut
                        'description' => '',
                        'is_manual' => true
                    ];
                }
            ...
            // Sauvegarder les hobbies manuels
            if (!empty($cvData['hobbies'])) {
                ...
                foreach ($cvData['hobbies'] as $hobbyName) {
                    ...
                    $manualHobbies[] = [
                        'id' => 'manual-' . uniqid(),
                        'name' => $hobbyName,
                        'name_en' => $hobbyName, // Même valeur par défaut
                        'is_manual' => true
                    ];
                }
            ...
            foreach ($cvData['experiences'] as $exp) {
                // Code existant pour les expériences...
    ...
    /**
     * Export avec support Median - GET endpoint pour téléchargement direct
     */
    public function exportDirect(Request $request)
    {
        try {
            ...
            if ($validated['format'] === 'pdf') {
                ...
                // Pour Median, forcer le téléchargement avec les bons headers
            ...
            if ($validated['format'] === 'pptx') {
                // PowerPoint se génère uniquement côté client
                return response()->json([
    ...
    /**
     * Endpoint d'impression direct pour mobile
     */
    public function printDirect(Request $request)
    {
        try {
            ...
            // Créer une vue optimisée pour l'impression mobile
    ...
    /**
     * Endpoint pour récupérer les données JSON d'une conversation pour PowerPoint
     */
    public function getPresentationData(Request $request)
    {
        try {
            $validated = $request->validate([
                'contextId' => 'required|string'
            ...
            // Trouver le dernier message de l'assistant avec du JSON
            $lastAiMessage = collect($messages)
                ->filter(fn($msg) => $msg['role'] === 'assistant')
                ->last();

            if ($lastAiMessage) {
                // Retourner le contenu brut pour que le client puisse extraire le JSON
                return response()->json([
                    'success' => true,
            ...
            return response()->json([
                'success' => false,
                'error' => 'Aucun message d\'assistant trouvé'
            ], 404);

        } catch (\Exception $e) {
            Log::error('Get presentation data error: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'error' => $e->getMessage()
            ], 500);
