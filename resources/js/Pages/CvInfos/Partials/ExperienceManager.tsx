...
const ExperienceManager: React.FC<Props> = ({ auth, experiences: initialExperiences, categories, onUpdate }) => {
    ...
    // Experience Card Component - Simplified for Mobile & Desktop
    const ExperienceCard: React.FC<{ experience: Experience }> = ({ experience: exp }) => {
        return (
            ...
            >
                <Card
                    className="border-amber-100 dark:border-amber-900/50 dark:bg-gray-800 hover:shadow-md transition-shadow cursor-pointer"
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center text-xs text-gray-600 dark:text-gray-300">
                        <BookOpen className="w-3 h-3 mr-1 flex-shrink-0 text-purple-500 dark:text-purple-400" />
                        <span>
                            {new Date(exp.date_start).toLocaleDateString('fr-FR', { year: '2-digit', month: '2-digit' })} -
                            {exp.date_end ? new Date(exp.date_end).toLocaleDateString('fr-FR', { year: '2-digit', month: '2-digit' }) : 'actuel'}
                        </span>
                ...
                <div className="space-y-1">
                    <div className="flex items-center text-sm text-gray-600 dark:text-gray-300">
                        <Building2 className="w-3 h-3 mr-1 flex-shrink-0 text-amber-500 dark:text-amber-400" />
                        <span className="font-medium">{exp.InstitutionName}</span>
                    </div>
                </div>

                <div className="space-y-2">
                    <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-200">Description</h4>
                ...
                {exp.output && (
                    <div className="space-y-2">
                        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-200">Résultat</h4>
                ...
                {exp.attachment_path && (
                    <div className="space-y-2">
                        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-200">Pièce jointe</h4>
                        <div className="flex items-center gap-2">
                ...
                <div className="flex justify-between gap-2 pt-4">
                    <Button
                        ...
                        className="border-amber-200 dark:border-amber-800 hover:bg-amber-50
                                 dark:hover:bg-amber-900/20 dark:text-white h-9 text-xs flex-1"
                    >
                    ...
                    <Button
                        ...
                        variant="outline"
                        className="border-red-200 dark:border-red-800 hover:bg-red-50
                                 dark:hover:bg-red-900/20 text-red-500 dark:text-red-400 h-9 text-xs flex-1"
                    >
    ...
    const maxNameLength = isMobile ? 5 : 10;

    return (
        <div className="space-y-3">
            {/* Header with Title and Add Button - Same for Mobile and Desktop */}
            <div className="flex justify-between items-center gap-2 mb-2">
                ...
                <Button
                    ...
                    className={`bg-gradient-to-r from-amber-500 to-purple-500
                             ...
                             text-white text-xs ${isMobile ? 'w-8 h-8 p-0' : 'h-8 py-0 px-2'}`}
                >
                    <Plus className="w-3 h-3" />
                    {!isMobile && <span className="ml-1">{t('experiences.actions.add')}</span>}
                </Button>
            </div>

            {/* Main tabs container */}
            <Tabs defaultValue="experiences" value={currentTab} onValueChange={setCurrentTab} className="w-full">
                {/* Header with tabs for Mobile - Remove the add button from here */}
                {isMobile && (
                    <TabsList className="grid w-full grid-cols-2 h-8 mt-2">
                        ...
                        <TabsTrigger value="attachments" className="flex items-center gap-1 text-[10px] py-0.5 h-7">
                            ...
                            {attachmentSummary.files_count > 0 && `(${attachmentSummary.files_count})`}
                        </TabsTrigger>
                    </TabsList>
                )}
                                        ...
                                        {experiences
                                            ...
                                            .map(exp => (
                                                <motion.div
                                                    ...
                                                    transition={{ duration: 0.2 }}
                                                >
                                                    <Card className="dark:bg-gray-800 cursor-pointer hover:shadow-md transition-shadow" onClick={() => handleOpenDetail(exp)}>
                                                        <CardContent className="p-2">
                                                            <div className="flex items-center gap-2">
